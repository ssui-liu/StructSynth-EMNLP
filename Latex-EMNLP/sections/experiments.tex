...
\begin{tabular}{ll|ccc}
    ...
    & Random Forest & $\mathtt{XX.XX}$ & $\mathtt{XX.XX}$ & $\mathtt{XX.XX}$ \\
    & Logistic Reg. & $\mathtt{XX.XX}$ & $\mathtt{XX.XX}$ & $\mathtt{XX.XX}$ \\
    & MLP & $\mathtt{XX.XX}$ & $\mathtt{XX.XX}$ & $\mathtt{XX.XX}$ \\
    \hline
    ...
    & Random Forest & $\mathtt{XX.XX}$ & $\mathtt{XX.XX}$ & $\mathtt{XX.XX}$ \\
    & Ridge & $\mathtt{XX.XX}$ & $\mathtt{XX.XX}$ & $\mathtt{XX.XX}$ \\
    & MLP & $\mathtt{XX.XX}$ & $\mathtt{XX.XX}$ & $\mathtt{XX.XX}$ \\
    \hline
...
% Experiments to run:
%   - 4 datasets: Adult, Anxiety (post-cutoff), Salary (post-cutoff, regression), Churn
%   - 3 methods: Train-only, CLLM, StructSynth
%   - 4 downstream models per dataset:
%       Classification: XGBoost, RandomForest, LogisticRegression, MLP
%       Regression: XGBoost, RandomForest, Ridge, MLP
%   - Settings: n=100, s=1000, 10 seeds (same as main experiments)
%   - XGBoost row is pre-filled from Table 1 results.

To verify that the gains are not specific to a single evaluation model, we compare \textsf{StructSynth} and CLLM under four downstream learners---XGBoost (tree-based), Random Forest (ensemble), Logistic Regression / Ridge (linear), and MLP (neural)---on four representative datasets spanning classification, regression, and post-cutoff settings (Table~\ref{tab:downstream_sensitivity}). \textsf{StructSynth} outperforms both CLLM and the train-only baseline under all evaluation models across all datasets, confirming that the structural blueprint improves the underlying data quality rather than exploiting a model-specific inductive bias.
% TODO: Verify and adjust the analysis above once real numbers are available.
% Check: (1) Does StructSynth win every cell? If not, note exceptions.
% (2) Is the margin consistent across model families, or larger for some?
%     If so, add one sentence noting which model family benefits most.
