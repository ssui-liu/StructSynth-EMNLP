...
[Submitted, §4.4, Figure 3, l.470–490] We evaluate Asia (8 nodes), Child (20), and Insurance (27) for \(n \in \{20,50,100,200\}\). Structural Hamming Distance counts the edge insertions, deletions, **and reversals** needed to recover the reference DAG; StructSynth attains the lowest or near-lowest SHD on all three, with its clearest advantage at \(n\leq 50\). [Submitted, Appendix D; Table 3] Directionality is functionally necessary because the DAG defines the topological generation schedule, but it is not an ontological or causal claim. Consistently, removing topological order lowers AUC by 1.1 points, while PC- and NoTears-discovered graphs lower it by 1.0 and 1.4 points.
...
We appreciate the request to clarify the method's operating range. **Direct answer.** StructSynth targets the \(n\leq 200\) low-data regime; not testing \(n>200\) is a scope boundary, and behavior beyond that range should be presented as a hypothesis rather than an established result.

[Submitted, §1, l.040–047; §4.5, Figure 4, l.496–505] From \(n=20\) to 200, StructSynth remains stable across utility, fidelity, and privacy, while its largest advantage occurs at \(n\leq 50\) and the gap narrows toward 200. [Submitted, §4.4, Figure 3] FCI and NoTears likewise approach StructSynth's SHD as \(n\) increases. [Submitted, Appendix K.3] The paper already identifies the method as best suited to severely limited data (especially \(n\leq100\)) and notes diminished advantages when statistical structure learning becomes reliable.

These observations motivate—but do not prove—the hypothesis that beyond \(n=200\), stronger association estimates will reduce the relative value of the LLM semantic prior while StructSynth's absolute performance remains competitive. We will state this expectation explicitly as a hypothesis.
...
**Revision (R2).** We will add the evaluated operating range, observed convergence trend, and clearly qualified \(n>200\) hypothesis to the Limitations section, with pointers to §4.4, §4.5, and Appendix K.3.
...
[Submitted, §3.1.2, l.249–258] StructSynth pairs semantic reasoning with observed association scores (Cramér's V, \(|r|\), and correlation ratio), so the statistical channel remains available when names are opaque. [New, Exp-2] In an exploratory run, we anonymized field names and task/column descriptions while keeping values unchanged *(gpt-5-mini, single seed 42, 100 real rows, 1,000 synthetic rows)*.
...
On Anxiety, anonymization produces a modest utility decrease (AUC 0.865→0.850) but better fidelity (0.579→0.534), suggesting that some semantically inferred edges may be spurious. On Salary, the original retains a larger advantage (R² 0.560 vs. 0.462), indicating that semantic priors contribute more to regression tasks involving fine-grained continuous dependencies. The result is mixed rather than uniformly robust.
...
These commitments make no claims beyond the submitted and new evidence above. If these responses resolve the concerns, we would be grateful if the reviewer would consider revisiting their assessment.
