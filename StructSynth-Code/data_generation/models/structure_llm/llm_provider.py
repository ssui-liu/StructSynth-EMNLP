...
from structure_learning.llm.utils import (
    VALID_PROVIDERS,
    openai_api_key,
    deepseek_api_key,
    qwen_api_key,
...
def create_provider(config: Dict[str, Any]) -> LLMProvider:
    ...
    if provider_type == "openai":
        return OpenAIProvider(model_name, temperature, api_key=api_key or openai_api_key) # Use global key if not in config
    elif provider_type == "deepseek":
        return DeepSeekProvider(model_name, temperature, api_key=api_key or deepseek_api_key) # Use global key if not in config
    elif provider_type == "qwen":
        return QwenProvider(model_name, temperature, api_key=api_key or qwen_api_key)
    elif provider_type == "llama":
        return OpenRouterProvider(model_name, temperature, api_key=api_key or openrouter_api_key)
    else:
...
class BaseLLMInterface:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.refinement_structured_model = self.provider.get_structured_model(RefinementResult)
