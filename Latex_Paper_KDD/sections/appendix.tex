...
The detailed hyperparameter configurations for the LLMs in \textsf{StructSynth}, the XGBoost model for downstream evaluation, and the Peter-Clark algorithm are provided in Table~\ref{tab:hyperparameters}. For the software implementation, we utilize the Langchain library to manage LLM inference and the Causal-Learn library~\cite{zheng2024causal} for the PC algorithm. 
...
\section{Association Score Computation}
\label{app:association_scores}

This section details the mathematical formulations for the three association measures used to quantify dependencies between attribute pairs. The choice of measure depends on the data types of the variables involved.
...
\begin{figure*}[h!]
    ...
    \begin{subfigure}[b]{0.37\textwidth}
        \centering
        \includegraphics[width=\linewidth]{fig/graph/reference.pdf}
    ...
    \begin{subfigure}[b]{0.3\textwidth}
        \centering
        \includegraphics[width=\linewidth]{fig/graph/llm.pdf}
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\linewidth]{fig/graph/structsynth.pdf}
...
\label{app:metrics}
\subsection{Downstream Model Performance}


The Downstream Model Performance metric serves as the primary measure of the synthetic data's practical utility. It evaluates whether the synthetic data, when used to augment the original training set, can improve the performance of a machine learning model on a real, unseen test set. A high score indicates that the generated data not only captures the underlying patterns of the original data but also enhances the model's ability to generalize.

\subsubsection{Evaluation Procedure}
For each generative method, we create an augmented training set, $\mathcal{D}_{\text{aug}}$, by combining the original limited training data, $\mathcal{D}_{\text{train}}$, with the generated synthetic data, $\mathcal{D}_{\text{synth}}$:
\begin{equation}
    \mathcal{D}_{\text{aug}} = \mathcal{D}_{\text{train}} \cup \mathcal{D}_{\text{synth}}
\end{equation}

A downstream machine learning model (in our experiments, an XGBoost model) is then trained exclusively on this augmented dataset, $\mathcal{D}_{\text{aug}}$. The performance of this trained model is subsequently evaluated on the held-out real test set, $\mathcal{D}_{\text{test}}$, which contains data that the model has not previously encountered during training.

\subsubsection{Performance Metrics}
The specific metric used for evaluation depends on the nature of the downstream task associated with each dataset:

For binary and multi-class classification tasks, we report the Area Under the Receiver Operating Characteristic Curve (AUC). The AUC score represents the model's ability to distinguish between different classes, with a value of 1.0 indicating a perfect classifier and 0.5 indicating performance equivalent to random chance. A higher AUC score is better.

For regression tasks, we report the R-squared ($R^2$) score, also known as the coefficient of determination. The $R^2$ score measures the proportion of the variance in the dependent variable that is predictable from the independent variables. An $R^2$ score of 1.0 indicates that the model perfectly predicts the target values, while a score of 0 indicates that the model performs no better than a simple mean-based prediction. A higher $R^2$ score is better.

\subsubsection{Interpretation}
Higher scores in either AUC or $R^2$ signify greater downstream utility. This indicates that the synthetic data has provided valuable and non-redundant information to the training process, enabling the machine learning model to learn a more robust and generalizable decision boundary or regression function.

\subsection{Privacy Risk Assessment Metric}
\label{app:privacy_metric}

The Privacy Risk Assessment metric evaluates the extent to which a generative model overfits or memorizes the training data, $\mathcal{D}_{\text{train}}$. A high degree of similarity between the synthetic data, $\mathcal{D}_{\text{synth}}$, and the training data suggests a potential privacy risk. The metric quantifies this risk by calculating the probability that the nearest neighbor of a synthetic data point, within the pool of all real data, originates from the training set. A value approaching 0.5 indicates robust privacy preservation, implying that the synthetic data is as close to the unseen test data as it is to the training data.

\subsubsection{Formal Definition}
Let $\mathcal{D}_{\text{train}}$ be the training set and $\mathcal{D}_{\text{test}}$ be the holdout test set. The complete set of real data instances is defined as $\mathcal{D}_{\text{real}} = \mathcal{D}_{\text{train}} \cup \mathcal{D}_{\text{test}}$. To ensure an unbiased comparison where the ideal probability is exactly 0.5, we first downsample the larger of the two sets ($\mathcal{D}_{\text{train}}$ or $\mathcal{D}_{\text{test}}$) to match the size of the smaller one, ensuring that $|\mathcal{D}_{\text{train}}| = |\mathcal{D}_{\text{test}}|$ for the purpose of this evaluation.
...
\subsubsection{Interpretation}
A score approaching 0.5 suggests that the generative model has not memorized the training data, as the synthetic samples are statistically equidistant to both the training and test sets. Conversely, a score approaching 1.0 indicates a significant privacy risk, as it implies that the synthetic data points are consistently closer to the training samples they were generated from than to unseen real data.

\subsection{Statistical Fidelity Metric}
\label{app:fidelity_metric}

The Statistical Fidelity metric quantifies how well the synthetic data, $\mathcal{D}_{\text{synth}}$, preserves the pairwise statistical relationships present in the real data, $\mathcal{D}_{\text{real}}$. The metric is computed as the mean absolute difference across all pairs of columns, where the specific correlation measure depends on the data types of the columns involved. A lower score indicates higher fidelity, signifying that the statistical dependencies between features in the synthetic data closely mirror those in the real data.

\subsubsection{Formal Definition}
For each unique pair of columns $(C_i, C_j)$ in the dataset, a difference score, $\Delta(C_i, C_j)$, is calculated. The calculation method varies for the three possible data type combinations:
...
\end{equation}
    ...
    \item {\textbf{Categorical-Categorical Pairs: } For two categorical columns, the difference is measured as the Total Variation Distance (TVD) of their joint probability distributions. This is calculated as half the sum of the absolute differences between the normalized contingency tables of the real and synthetic data:
\begin{align}
...
% \subsubsection{Interpretation}
% A score approaching 0 indicates high statistical fidelity, as it implies that the pairwise relationships between columns in the synthetic data are nearly identical to those in the real data. Conversely, a higher score signifies a greater divergence in these statistical properties.


\section{Algorithms}
\label{app:algorithm}
We show the detailed algorithm of our \textsf{StructSynth} in Algorithm \ref{alg:structsynth}.


\section{Qualitative Graph Visualization on Adult Dataset}
...
\section{Structure Learning Examples}
\label{app:struct_example}
\begin{figure*}[t!]
\centering
\includegraphics[width=1.8\columnwidth]{fig/collage_3x2.png}
...
To provide a concrete illustration of the structural discovery capabilities discussed in Section 4.4, Figure \ref{app:structure_examples} presents a visual comparison of the dependency graphs learned by \textsf{StructSynth} and four representative baselines on the Asia dataset ($n=100$). The \textbf{Ground Truth} graph represents the underlying causal mechanism of the dataset, where, for example, \textit{smoke} causes \textit{lung} (cancer) and \textit{bronc} (bronchitis), which in turn cause \textit{dysp} (dyspnoea).
...
\section{Token Usage Analysis}
\label{sec:token_usage}

Table~\ref{tab:token_usage} reports the token consumption of \textsc{CLLM} and \textsf{StructSynth} under the \textbf{100-shot} setting, averaged over six real-world datasets. Overall, introducing the graph-based structure modeling in \textsf{StructSynth} does \emph{not} lead to a prohibitive cost increase: the average total usage rises from \textbf{219.8K} tokens (\textsc{CLLM} total) to \textbf{287.9K} tokens (\textsf{StructSynth} combined total), corresponding to an absolute overhead of \textbf{+68.1K} tokens (about \textbf{+31.0\%}). This overhead is relatively stable across datasets (roughly \textbf{+19\%} to \textbf{+42\%} in total), suggesting that the additional structure reasoning introduces a consistent but moderate computational budget.

A key observation is that the overhead is \textbf{highly skewed toward the input side}. Concretely, \textsf{StructSynth} increases average \emph{input} tokens by \textbf{+65.1K} (\textbf{+57.1\%}), while the \emph{output} tokens only increase by \textbf{+2.9K} (\textbf{+2.7\%}). This pattern is expected: \textsf{StructSynth} needs additional prompt context to (i) elicit and refine the dependency graph and (ii) condition the synthesis stage on the discovered structure, whereas the corresponding model responses (e.g., edge lists / compact graph representations) are short and the final table synthesis outputs remain comparable in length to \textsc{CLLM}. Operationally, most LLM services track and bill \emph{prompt (input) tokens} and \emph{completion (output) tokens} separately.
...
\For{$j \gets 1$ to $s$}
    ...
    \State $\tilde{x}_{j, A_{\mathtt{iso}}} \gets f_{\mathtt{LLM}}(\pi_{\mathtt{data\_gen\_iso}}(\tilde{x}_{j,V}, A_{\mathtt{iso}}, \mathcal{D}_{\mathtt{few\_shot}}^{\mathtt{iso}}))$
